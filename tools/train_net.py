...
@click.command()
@click.option('--conf', type=str, default='config.yaml', help="configuration file of siamfcos tracker")
@click.option('--seed', type=int, default=12345678, help='initial random seed, default=123456')
@click.option('--local-rank', type=int, default=0, help="specify a device to run pytorch launcher")
def main(conf, seed, local_rank):
    # initial device random status by seed
    seed_torch(seed)
    ...
    # load cfg
    # if()
    if(opath.exists(conf)):
        cfg.merge_from_file(conf)

    if rank == 0:
        if not opath.exists(cfg.TRAIN.LOG_DIR):
            os.makedirs(cfg.TRAIN.LOG_DIR)
        init_log('global', logging.INFO)
        if cfg.TRAIN.LOG_DIR:
            add_file_handler('global',
                             opath.join(cfg.TRAIN.LOG_DIR, 'logs.txt'),
                             logging.INFO)
