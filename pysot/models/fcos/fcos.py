...
class FCOSHead(nn.Module):
    ...
    def forward(self, x):
        ...
        for idx, feature in enumerate(x):
            ...
            bbox_reg.append(
                torch.exp(self.scales[idx](
                    self.bbox_pred(
                        self.bbox_tower(feature))
                    )))
        return logits, feature, centerness


class FCOSModule(nn.Module):
    '''
    Module for FCOS computation. 
    ...
    def _forward_train(self, locations, box_cls, box_regression, centerness, targets):
        ...
        losses = {
            ...
            "loss_centernerss": loss_centerness
        }
        return None, losses
    ...
    def compute_locations_per_level(self, h, w, stide, device):
